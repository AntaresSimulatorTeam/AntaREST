...
export const updateUpload = (id: string, completion: number): UpdateUploadAction => ({
  type: 'UPLOAD/UPDATE',
...
export const completeUpload = (id: string): ThunkAction<void, AppState, unknown, AnyAction> => (dispatch, getState): void => {
...
type UploadAction = AddUploadAction | UpdateUploadAction | CompleteUploadAction | UploadGlobalListenerAction;
  ...
  switch (action.type) {
    ...
    case 'UPLOAD/UPDATE': {
      ...
      if (upload) {
        ...
        return {
          ...state,
          uploads: state.uploads.filter((u) => u.id !== action.payload.id).concat(upload),
        };
      }
      return state;
    }
