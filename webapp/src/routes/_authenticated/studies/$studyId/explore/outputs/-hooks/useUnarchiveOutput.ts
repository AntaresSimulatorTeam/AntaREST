...
function useUnarchiveOutput() {
  ...
  const mutation = useMutation({
    ...
    onMutate: async (variables) => {
      ...
      const outputToUnarchive = prevOutputs?.find((output) => output.id === outputId);

      if (outputToUnarchive) {
        queryClient.setQueryData(queryListKey, (old = []) => {
          return old.map((output) =>
            output.id === outputId
              ? createOptimisticListItem({ ...output, archived: false })
              : output,
          );
        });
      }
    ...
    onError: (error, variables, onMutateResult) => {
      ...
      if (outputToUnarchive) {
        queryClient.setQueryData(queryListKey, (old = []) =>
          old.map((output) => (output.id === outputId ? outputToUnarchive : output)),
        );
      }

      enqueueErrorSnackbar(
        t("studies.error.unarchiveOutput", {
          outputName:
...
export default useUnarchiveOutput;
