...
export type DialogType = "launch" | "properties" | "upgrade" | "export" | "move" | "delete";
...
function StudyActionsMenu({ open, anchorEl, onClose, study, parentStudy, hasVariants }: Props) {
  const { t } = useTranslation();
  const enqueueErrorSnackbar = useEnqueueErrorSnackbar();
  const [openDialog, setOpenDialog] = useState<DialogType | null>(null);
  const deleteAction = useConfirm<{ includesChildren: boolean }>();
    ...
    deleteAction.showConfirm().then((confirm) => {
      if (confirm) {
        dispatch(
          deleteStudy({
            id: study.id,
            deleteChildren: deleteAction.data?.includesChildren ?? false,
          }),
        )
          .unwrap()
          .then(() => {
    ...
    <>
      <Menu open={open} anchorEl={anchorEl} keepMounted onClose={onClose}>
        {[
          ...
          menuItem(
            !isArchived && !isLatestVersion && !isVariant && !hasVariants,
            t("study.upgrade"),
            UpgradeIcon,
            "upgrade",
          ),
      ...
      <ConfirmationDialog
        ...
        alert="warning"
        disableConfirm={hasVariants && !deleteAction.data?.includesChildren}
      >
        <Box
          sx={{
            display: "flex",
        ...
        >
          ...
          {hasVariants && (
            <CheckBoxFE
              ...
              onChange={(_, checked) => deleteAction.setData({ includesChildren: checked })}
            />
          )}
        </Box>
      </ConfirmationDialog>
