...
export default function CustomizedTreeView(props: Props) {
  ...
  const buildRecursiveTree = (tree: StudyTree, i = 0, j = 0) : Array<React.ReactNode> => {
    ...
    let res : Array<React.ReactNode> = [<SVGCircle key={`circle-${i}-${j}`} cx={cx} cy={cy} r={CIRCLE_RADIUS} fill={color} />,
      <SVGRect key={`rect-${i}-${j}`} x="0" y={cy - TILE_SIZE_Y_2 + RECT_Y_SPACING_2} width={rectWidth} height={TILE_SIZE_Y - RECT_Y_SPACING} fill={hoverId === id || study?.id === id ? rectHoverColor : rectColor} onClick={() => onClick(id)} onMouseOver={(e) => onMouseOver(id)} onMouseOut={onMouseOut} />,
      <SVGRect key={`rect-for-name-${i}-${j}`} x={rectWidth + RECT_X_SPACING} y={cy - TILE_SIZE_Y_2 + RECT_Y_SPACING_2} width={RECT_TEXT_WIDTH} height={TILE_SIZE_Y - RECT_Y_SPACING} fill={hoverId === id || study?.id === id ? hoverColor : rectColor} onClick={() => onClick(id)} onMouseOver={(e) => onMouseOver(id)} onMouseOut={onMouseOut} />,
      ...
      <SVGText key={`name-${i}-${j}`} x={rectWidth + RECT_X_SPACING + RECT_DECORATION + TEXT_SPACING} y={cy + RECT_Y_SPACING_2} fill={hoverId === id || study?.id === id ? 'black' : 'white'} fontSize={TEXT_SIZE} onClick={() => onClick(id)} onMouseOver={(e) => onMouseOver(id)}>{name}</SVGText>];
