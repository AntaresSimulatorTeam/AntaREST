...
def create_event_bus(app_ctxt: t.Optional[AppBuildContext], config: Config) -> Tuple[IEventBus, t.Optional[redis.Redis]]:  # type: ignore
...
def create_services(config: Config, app_ctxt: t.Optional[AppBuildContext], create_all: bool = False) -> t.Dict[str, t.Any]:
...
def from_json(data: t.Union[str, bytes, bytearray]) -> t.Dict[str, t.Any]:
    adapter = pydantic.TypeAdapter(type=t.Any, config=pydantic.config.ConfigDict(ser_json_inf_nan="constants"))
    return adapter.json_schema(data)  # type: ignore


def to_json(data: t.Dict[str, t.Any], file: t.Any) -> None:
    adapter = pydantic.TypeAdapter(type=t.Any, config=pydantic.config.ConfigDict(ser_json_inf_nan="constants"))
    file.write(adapter.dump_json(data))  # type: ignore
