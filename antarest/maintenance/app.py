...
def _setup_periodic_tasks(sender: Celery, **_: object) -> None:
    ...
    variable_view_gc_interval = config.storage.variable_view_gc_sleeping_time if config else 3600

    logger.info(
        f"Periodic tasks configured successfully "
        f"(matrix_gc_interval={matrix_gc_interval}s, variable_view_gc_interval={variable_view_gc_interval}s)"
    )
    ...
    sender.add_periodic_task(
        storage.variable_view_gc_sleeping_time, clean_variable_views_task.s(), name="variable_views_cleaner"
    )
