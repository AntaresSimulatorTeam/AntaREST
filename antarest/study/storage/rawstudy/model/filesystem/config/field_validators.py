...
def validate_filtering(v: t.Any) -> str:
    ...
    if isinstance(v, str):
        values = list(set(_find_all_filtering(v.lower())))
        values.sort(key=lambda x: _ALL_FILTERING.index(x))
        return ", ".join(values)
    elif isinstance(v, (list, tuple)):
        values = list(set(_find_all_filtering(x.lower() for x in v)))  # type: ignore
        values.sort(key=lambda x: _ALL_FILTERING.index(x))
        return ", ".join(values)
    else:
...
def validate_color_rgb(v: t.Any) -> t.Tuple[int, int, int]:
    """
    Validate RGB color field and convert it to a tuple of 3 integers.
