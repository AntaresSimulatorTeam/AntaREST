...
from antarest.study.storage.rawstudy.model.filesystem.config.district import DistrictSet, area_sign_from_data
...
def _parse_sets(root: Path) -> Dict[str, DistrictSet]:
    ...
    return {
        transform_name_to_id(name): DistrictSet(
            areas=item.get(area_sign_from_data(item)),
            name=item.get("caption"),
            inverted_set=area_sign_from_data(item) == "-",
