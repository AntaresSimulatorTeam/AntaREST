...
class ClusteredRenewables(FolderNode):
    ...
    def build(self) -> TREE:
        children: TREE = {
            "clusters": ClusteredRenewableAreaCluster(self.matrix_storage_context, self.config.next_file("clusters")),
            "series": ClusteredRenewableAreaSeries(self.matrix_storage_context, self.config.next_file("series")),
        }
