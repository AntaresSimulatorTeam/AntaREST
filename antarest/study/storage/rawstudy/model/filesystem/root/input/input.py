...
class Input(FolderNode):
    ...
    def build(self) -> TREE:
        ...
        children: TREE = {
            "areas": InputAreas(self.matrix_storage_context, config.next_file("areas")),
            "bindingconstraints": BindingConstraints(
                self.matrix_storage_context, config.next_file("bindingconstraints")
            ),
            "hydro": InputHydro(self.matrix_storage_context, config.next_file("hydro")),
            "links": InputLink(self.matrix_storage_context, config.next_file("links")),
            "load": InputPreproSeries(self.matrix_storage_context, config.next_file("load"), "load_"),
            "misc-gen": InputMiscGen(self.matrix_storage_context, config.next_file("misc-gen")),
            "reserves": InputReserves(self.matrix_storage_context, config.next_file("reserves")),
            "solar": InputPreproSeries(self.matrix_storage_context, config.next_file("solar"), "solar_"),
            "thermal": InputThermal(self.matrix_storage_context, config.next_file("thermal")),
            "wind": InputPreproSeries(self.matrix_storage_context, config.next_file("wind"), "wind_"),
        }
        ...
        if has_renewables:
            children["renewables"] = ClusteredRenewables(self.matrix_storage_context, config.next_file("renewables"))

        if study_version >= STUDY_VERSION_8_6:
            children["st-storage"] = InputSTStorage(self.matrix_storage_context, config.next_file("st-storage"))
