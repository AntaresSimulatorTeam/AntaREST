...
class FileStudyTree(FolderNode):
    ...
    def build(self) -> TREE:
        children: TREE = {
            ...
            "settings": Settings(self.matrix_storage_context, self.config.next_file("settings")),
            "layers": Layers(self.matrix_storage_context, self.config.next_file("layers")),
            "input": Input(self.matrix_storage_context, self.config.next_file("input")),
            "user": User(self.matrix_storage_context, self.config.next_file("user")),
        }

        if (self.config.path / "logs").exists():
            children["logs"] = BucketNode(self.matrix_storage_context, self.config.next_file("logs"))
        ...
        if self.config.outputs:
            ...
            children["output"] = Output(self.matrix_storage_context, output_config)
