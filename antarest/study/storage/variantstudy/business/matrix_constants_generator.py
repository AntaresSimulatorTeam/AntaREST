...
class GeneratorMatrixConstants:
    ...
    def init_constant_matrices(self, bucket_dir: Path) -> None:
        ...
        with FileLock(bucket_dir / MATRIX_CONSTANT_INIT_LOCK_FILE_NAME):
            ...
            self.hashes[BINDING_CONSTRAINT_HOURLY] = self.matrix_service.create(
                series.default_binding_constraint_hourly
            )
            self.hashes[BINDING_CONSTRAINT_DAILY] = self.matrix_service.create(series.default_binding_constraint_daily)
            self.hashes[BINDING_CONSTRAINT_WEEKLY] = self.matrix_service.create(
                series.default_binding_constraint_weekly
            )
