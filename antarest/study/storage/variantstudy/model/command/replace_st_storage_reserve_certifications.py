...
class ReplaceStStorageReserveCertifications(ICommand):
    """
    Command used to replace reserve certifications for a given area
    """

    command_name: CommandName = CommandName.REPLACE_ST_STORAGE_RESERVE_CERTIFICATIONS

    # Command parameters
    # ==================

    area_id: AreaId
    certifications: StorageReserveCertificationMapping
    ...
    ) -> CommandOutput[StorageReserveCertificationMapping]:
        study_data.save_st_storage_reserve_certifications({self.area_id: self.certifications})
    ...
    def to_dto(self) -> CommandDTO:
        args: dict[str, Any] = {}
        for reserve_id, st_storage_dict in self.certifications.items():
            for storage_id, certification in st_storage_dict.items():
                args.setdefault(reserve_id, {})[storage_id] = certification.model_dump(mode="json")
