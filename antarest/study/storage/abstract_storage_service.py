...
class AbstractStorageService(IStudyStorageService[T], ABC):
    ...
    def archive_study_output(self, study: T, output_id: str) -> bool:
        ...
        try:
            ...
            with py7zr.SevenZipFile(Path(study.path) / "output" / f"{output_id}.7z", "w") as szf:
                szf.writeall(Path(study.path) / "output" / output_id, arcname=".")
            if (Path(study.path) / "output" / f"{output_id}.7z").exists():
                shutil.rmtree(Path(study.path) / "output" / output_id)
            remove_from_cache(self.cache, study.id)
            return True
        except Exception as e:
            logger.warning(
