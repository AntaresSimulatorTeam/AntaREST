...
class DatabaseStudyStorage(IStudyStorage):
    ...
    def upgrade_study(self, study: Study, version: StudyVersion) -> None:
        ...
        # If this operation fails, we should rollback the study in the database, otherwise we'd have a corrupted study.
        # As the study converter uses multiple commits, we cannot simply rollback the `db.session`
        # So, we have to do the following things:
        # - Create a study with a new id based on the FS
        # - If it fails, we simply remove this temporary study from DB
        # - If it succeeds, we replace the original study data with the new data and there will be almost no reason for this to fail.
        # And then we remove the temporary study from the DB

        temporary_study_id = str(uuid.uuid4())
        try:
            study.version = f"{version:2d}"
            self._repository.save(study)
        except Exception as e:
            # todo: Here we should rollback the study in the database if something happened ...
            raise e
        finally:
            shutil.rmtree(dst_path, ignore_errors=True)
