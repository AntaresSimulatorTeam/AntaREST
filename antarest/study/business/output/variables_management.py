...
class OutputVariablesManager:
    ...
    def _get_all_headers_and_file_type(
        ...
        """
        filtered_files = self._filter_files_with_same_prefix(parent_path, file_type_class)
        for file_type, freq in filtered_files.items():
            file_path = parent_path / f"{file_type}-{freq.value}.txt"
            yield self._read_headers_only(file_path, mc_root, freq, file_type), file_type
        ...
        for mc_path in existing_paths:
            ...
            if areas_folder.exists():
                for area in sorted(areas_folder.iterdir()):
                    ...
                    for cols, file_type in self._get_all_headers_and_file_type(mc_root, parent_path, file_type_class):
            ...
            if links_folder.exists():
                for link_name in sorted(links_folder.iterdir()):
                    ...
                    for cols, _ in self._get_all_headers_and_file_type(mc_root, parent_path, file_type_klass):
