...
class FileStudyConstraintDao(ConstraintDao, ABC):
    ...
    def save_constraints(self, constraints: Sequence[BindingConstraint]) -> None:
        ...
        # Loads old constraints before saving new ones
        # We need to do this because the tree is based on the config, and the config is based on the tree
        # We also need to keep track of the nodes to update them later
        bc_id_to_old_bc_nodes = self._get_all_constraints_from_tree(bc_id_to_bc_object, study_data)

        self._update_ini_file(bc_id_to_key_in_ini, constraints)

        for constraint in constraints:
            bc_id = constraint.id
            if constraint.id in bc_id_to_bc_object:
                self._update_existing_constraint(
                    bc_id_to_bc_object, constraint, bc_id_to_old_bc_nodes[bc_id], old_groups
                )

            elif constraint.group:
                # We're creating a new constraint with a non-None group
...
def update_matrices_names(
    ...
    Args:
        constraint_node: the tree nodes representing the previous binding constraint operators
