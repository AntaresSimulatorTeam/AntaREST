...
def create_study_data_routes(study_service: StudyService, config: Config) -> APIRouter:
    ...
    def generate_timeseries(uuid: str, thermal_outage_details: bool = Query(default=False)) -> str:
        """
        Generates time-series for thermal clusters and put them inside input data.

        Args:
        - `uuid`: The UUID of the study.
        - `thermal_outage_details`: Whether to generate thermal outage details.
        """
        logger.info(f"Generating timeseries for study {uuid}")
        study = study_service.check_study_access(uuid, StudyPermissionType.WRITE)
        return study_service.generate_timeseries(study, thermal_outage_details)
