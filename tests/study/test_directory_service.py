...
class TestDirectoryService:
    ...
    def test_build_folder_path_from_directory_simple(
        self, directory_service: DirectoryService, mock_directory_repo: Mock, test_user: Identity
    ) -> None:
        # Setup
        ...
        # Execute
        result = directory_service.build_folder_path_from_directory(directory_id, study_id)

        # Verify
        assert result == f"project/{study_id}"

    def test_build_folder_path_from_directory_nested(
        self, directory_service: DirectoryService, mock_directory_repo: Mock, test_user: Identity
    ) -> None:
        # Setup
        study_id = str(uuid.uuid4())
        ...
        root = Directory(id=root_id, name="project", parent_id=None)
        child = Directory(id=child_id, name="subfolder", parent_id=root_id)
        grandchild = Directory(id=grandchild_id, name="deep", parent_id=child_id)

        # Mock repository to return directories in order as we traverse up
        def get_by_id_side_effect(directory_id: str):
            if directory_id == grandchild_id:
                return grandchild
            elif directory_id == child_id:
                return child
            elif directory_id == root_id:
                return root
            return None

        mock_directory_repo.get_by_id.side_effect = get_by_id_side_effect

        # Execute
        result = directory_service.build_folder_path_from_directory(grandchild_id, study_id)

        # Verify
        assert result == f"project/subfolder/deep/{study_id}"
