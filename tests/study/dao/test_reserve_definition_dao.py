...
def test_save_and_retrieve(dao_10_2: StudyDao) -> None:
    ...
    dao_10_2.save_reserve_definitions({"paris": [reserve]})

    fetched = dao_10_2.get_reserve_definition("paris", reserve.id)
...
def test_save_raises_area_not_found_when_area_missing(dao_10_2: StudyDao) -> None:
    with pytest.raises(AreaNotFound):
        dao_10_2.save_reserve_definitions({"nonexistent": [_reserve("R1")]})


def test_reserve_definition_exists(dao_10_2: StudyDao) -> None:
    save_area(dao_10_2, "paris")
...
def test_get_raises_reserve_not_found_when_area_exists(dao_10_2: StudyDao) -> None:
    ...
    with pytest.raises(ReserveDefinitionNotFound):
        dao_10_2.get_reserve_definition("paris", "unknown")


def test_get_all_for_area(dao_10_2: StudyDao) -> None:
    save_area(dao_10_2, "paris")
    dao_10_2.save_reserve_definitions({"paris": [_reserve("R1"), _reserve("R2", ReserveType.DOWN)]})
...
def test_get_all_for_area_empty(dao_10_2: StudyDao) -> None:
    save_area(dao_10_2, "paris")
    assert list(dao_10_2.get_all_reserve_definitions_for_area("paris")) == []


def test_get_all_for_area_raises_area_not_found(dao_10_2: StudyDao) -> None:
    with pytest.raises(AreaNotFound):
        list(dao_10_2.get_all_reserve_definitions_for_area("nonexistent"))


def test_get_all_across_areas(dao_10_2: StudyDao) -> None:
    save_area(dao_10_2, "paris")
    save_area(dao_10_2, "lyon")

    dao_10_2.save_reserve_definitions(
        {
...
def test_get_all_reserve_needs_empty(dao_10_2: StudyDao) -> None:
    save_area(dao_10_2, "paris")
    assert dao_10_2.get_all_reserve_needs() == {}
...
def test_cascade_delete_on_area_removal(dao_10_2: StudyDao) -> None:
    ...
    dao_10_2.delete_area("paris")

    assert dao_10_2.get_all_reserve_definitions() == {}


def test_removing_a_thermal_reserve_cascades_on_symmetries_and_certifications(dao_10_2: StudyDao) -> None:
...
class TestCoexistenceWithGlobalParameters:
    ...
    def test_get_all_excludes_global_parameters_section(self, dao_10_2: StudyDao) -> None:
        save_area(dao_10_2, "paris")
        dao_10_2.save_reserves_global_parameters(
            {"paris": ReservesGlobalParameters(reference_activation_duration_up=7)}
        )
        dao_10_2.save_reserve_definitions({"paris": [_reserve("Reserve 1"), _reserve("Reserve 2", ReserveType.DOWN)]})

        reserves = list(dao_10_2.get_all_reserve_definitions_for_area("paris"))
        ids = sorted(r.id for r in reserves)
        assert ids == ["reserve 1", "reserve 2"]
        assert "globalparameters" not in ids

    def test_save_reserve_preserves_global_parameters(self, dao_10_2: StudyDao) -> None:
        save_area(dao_10_2, "paris")
    ...
    def test_save_global_parameters_preserves_reserves(self, dao_10_2: StudyDao) -> None:
        save_area(dao_10_2, "paris")
        reserve = _reserve("R1")
        dao_10_2.save_reserve_definitions({"paris": [reserve]})
        dao_10_2.save_reserves_global_parameters(
