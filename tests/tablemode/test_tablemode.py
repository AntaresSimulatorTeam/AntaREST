...
class TestTablemode:
    ...
    def test_add_tablemode_failure_invalid_table_data(self):
        ...
        with pytest.raises(
            HTTPException,
            match=f"400: Invalid columns for table type {my_table_type}: " + ", ".join(invalid_table_columns),
        ):
    ...
    def test_update_tablemode_failure_updating_tablemode_with_invalid_table_data(self):
        ...
        with pytest.raises(
            HTTPException,
            match=f"400: Invalid columns for table type {my_table_type}: {', '.join(incorrect_table_columns)}",
        ):
