...
def test_hooks(tmp_path: Path):
    ...
    study_service.storage_service.raw_study_service.study_factory.create_from_fs.return_value = filestudy
    ...
    with pytest.raises(AssertionError):
        adq_ext.after_export_flat_hook("some-job", "some-study-id", study_export_path, {})

    adq_ext.after_export_flat_hook("some-job", "some-study-id", study_export_path, {})
    assert (study_export_path / "post-processing.R").exists()
    assert (study_export_path / "user" / "adequacypatch" / "hourly-areas.yml").exists()
    assert (study_export_path / "user" / "adequacypatch" / "year-by-year-active").exists()
