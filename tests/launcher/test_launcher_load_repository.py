...
class TestLauncherLoadRepository(LauncherLoadRepository):
...
@with_db_context
def test_database_launcher_load_is_empty_by_default() -> None:
    db_launcher_load = TestLauncherLoadRepository()
...
def test_should_be_able_to_add_launcher_load_data_to_db() -> None:
    db_launcher_load = TestLauncherLoadRepository()
    db_launcher_load.update_all_launcher_loads([launcher_load_1])

    actual_launchers_cache = db_launcher_load.get_launchers_loads()
    assert len(actual_launchers_cache) == 1

    check_launcher_load_equals(actual_launchers_cache[0], launcher_load_1)


@with_db_context
def test_should_be_able_to_update_launcher_load_data_from_db() -> None:
    db_launcher_load = TestLauncherLoadRepository()
    db_launcher_load.update_all_launcher_loads([launcher_load_2])

    actual_launchers_cache = db_launcher_load.get_launchers_loads()
    assert len(actual_launchers_cache) == 1

    check_launcher_load_equals(actual_launchers_cache[0], launcher_load_2)

    db_launcher_load.update_all_launcher_loads([launcher_load_2_updated])
    actual_launchers_cache = db_launcher_load.get_launchers_loads()
    assert len(actual_launchers_cache) == 1

    check_launcher_load_equals(actual_launchers_cache[0], launcher_load_2_updated)


@with_db_context
def test_should_be_able_to_update_multiple_launchers_cache_data_from_db() -> None:
    db_launcher_load = TestLauncherLoadRepository()
    db_launcher_load.update_all_launcher_loads([launcher_load_1, launcher_load_2])
...
@with_db_context
def test_should_update_all_launcher_load_data_from_db() -> None:
    db_launcher_load = TestLauncherLoadRepository()
    db_launcher_load.update_all_launcher_loads([launcher_load_1, launcher_load_2])
    ...
    db_launcher_load.update_all_launcher_loads([launcher_load_1, launcher_load_2_updated])
...
def assert_launcher_load_is_in_db(
    expected_launcher_load: LauncherLoad, db_launcher_load: TestLauncherLoadRepository
) -> None:
    actual_launchers_cache = db_launcher_load.get_launchers_loads()

    actual_cache = [
        load for load in actual_launchers_cache if load.launcher_name == expected_launcher_load.launcher_name
    ]
    assert len(actual_cache) == 1
    check_launcher_load_equals(actual_cache[0], expected_launcher_load)
