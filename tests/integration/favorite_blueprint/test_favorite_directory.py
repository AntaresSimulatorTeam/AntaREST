...
def test_favorite_directory(client: TestClient, admin_access_token: str) -> None:
    ...
    dto_test_1 = {"directory_id": dt_1_id, "directory_name": "directory_test_1"}
    dto_test_2 = {"directory_id": dt_2_id, "directory_name": "directory_test_2"}

    resp = client.post(f"/v1/favorite/directories/{dt_1_id}")
    assert resp.status_code == 200
    directory_dto_1 = resp.json()
    assert directory_dto_1["directory_id"] == dt_1_id
    assert directory_dto_1["directory_name"] == "directory_test_1"

    resp = client.post(f"/v1/favorite/directories/{dt_2_id}")
    assert resp.status_code == 200
    directory_dto_2 = resp.json()
    assert directory_dto_2["directory_id"] == dt_2_id
    assert directory_dto_2["directory_name"] == "directory_test_2"
...
def test_delete_directory(client: TestClient, admin_access_token: str) -> None:
    ...
    resp = client.post(f"/v1/favorite/directories/{dt_1_id}")
    assert resp.status_code == 200
    assert resp.json()["directory_id"] == dt_1_id
    resp = client.post(f"/v1/favorite/directories/{dt_2_id}")
    assert resp.status_code == 200
    assert resp.json()["directory_id"] == dt_2_id

    actual_favorite_directories = client.get("/v1/favorite/directories").json()
...
def test_add_inexisting_directory_failure(client: TestClient, admin_access_token: str) -> None:
    ...
    resp_add = client.post(f"/v1/favorite/directories/{inexisting_directory_id}")
