...
def test_favorite(client: TestClient, admin_access_token: str) -> None:
    ...
    resp = client.post(f"/v1/favorite/{study_test_1}")
    assert resp.status_code == 200
    resp = client.post(f"/v1/favorite/{study_test_2}")
    assert resp.status_code == 200
    ...
    favorite_list = client.get("/v1/favorite").json()
    assert favorite_list == expected_favorite_list

    # deleting studies from favorites

    client.delete(f"/v1/favorite/{study_test_1}")
    favorite_list = client.get("/v1/favorite").json()
    assert favorite_list == [fav_study_dto_2]

    client.delete(f"/v1/favorite/{study_test_2}")
    favorite_list = client.get("/v1/favorite").json()
