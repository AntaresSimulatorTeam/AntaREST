...
def test_nominal_case_of_an_api_user(client: TestClient, admin_access_token: str) -> None:
    ...
    client.headers = bot_headers

    # import a study
    res = client.post("/v1/studies/_import", files={"study": io.BytesIO(study_path.read_bytes())})
    study_id = res.json()

    # create a variant from it
    res = client.post(f"/v1/studies/{study_id}/variants", params={"name": "foo"})
    variant_id = res.json()
    ...
    res = client.put(f"/v1/studies/{variant_id}/config/general/form", json={"yearByYear": True})
    assert res.status_code == 200
    commands = [
        {
            "action": "update_config",
            "args": {"target": f"input/areas/{area_id}/optimization/filtering/filter_synthesis", "data": "annual"},
        }
    ]
    ...
    res = client.put(f"/v1/studies/{variant_id}/config/general/form", json={"nbYears": 10})
    assert res.status_code == 200
    res = client.put(
        f"/v1/studies/{variant_id}/config/playlist/form", json={1: {"status": False}, 4: {"weight": 12}, 7: {}}
    )
    assert res.status_code == 200
    ...
    commands = [
        {
            ...
            "args": {
                ...
                "parameters": {"group": "Gas", "unitcount": 1, "marginal_cost": 50},
            },
        }
    ]
    res = client.post(f"/v1/studies/{variant_id}/commands", json=commands)
    ...
    commands_res = client.get(f"/v1/studies/{variant_id}/commands")

    for command in commands_res.json():
        assert command["user_name"] == "admin"  # Should be the user's name not the token name
        assert command["updated_at"]

    # generate variant before running a simulation
    res = client.put(f"/v1/studies/{variant_id}/generate")
    assert res.status_code == 200
    res = client.get(f"/v1/tasks/{res.json()}", params={"wait_for_completion": True})
    assert res.status_code == 200
    ...
    res = client.post(f"/v1/launcher/run/{variant_id}", json=launcher_options)
    ...
    def wait_unit_finished() -> bool:
        res_ = client.get(f"/v1/launcher/jobs/{job_id}")
    ...
    assert res.json() == {"columns": ANY, "data": ANY, "index": ANY}
