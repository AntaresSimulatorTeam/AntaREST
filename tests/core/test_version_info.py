...
class TestVersionInfo:
    ...
    def test_get_commit_id__commit_id__exist(self, tmp_path: Path) -> None:
    ...
    def test_get_commit_id__git_call_ok(self, tmp_path: Path) -> None:
        actual = get_commit_id(tmp_path)
        assert re.fullmatch(r"[0-9a-fA-F]{40}", actual)

    def test_get_commit_id__git_call_failed(self, tmp_path: Path) -> None:
